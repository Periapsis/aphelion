...
public class TileAsteroidSmallA extends TileType
{
        private AsyncTexture texture;
        private Animation anim;
        
        public TileAsteroidSmallA(ResourceDB db, short tileID)
        {
                super(db, tileID);
                this.layer = TILE_LAYER.ANIMATED;
                if (db != null)
                {
                        texture = db.getTextureLoader().getTexture("mapclassic.asteroid.small1");
                }
        }
        ...
        public void render(Camera camera, int tileX, int tileY, aphelion.shared.map.MapClassic map)
        {
                if (texture.isLoaded())
                {
                        ...
                        {
                        	//Classic
                                //anim = SwissArmyKnife.spriteToAnimation(texture.getCachedImage(), 15, 2, 60);
                            
                        	//JabBase
                        	anim = SwissArmyKnife.spriteToAnimation(texture.getCachedImage(), 8, 8, 365 / 6);
                        }
                }
                else
                {
