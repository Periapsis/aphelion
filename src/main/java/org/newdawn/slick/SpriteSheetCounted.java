 ...
 * @author Kevin Glass
 */
public class SpriteSheetCounted extends Image
{
	 ...
	 */
	public SpriteSheetCounted(Image image,int tw,int th) {
		super(image);
	 ...
	 */
	public SpriteSheetCounted(Image image,int tw,int th,int spacing,int margin) {
		super(image);
	 ...
	 * @param spacing The spacing between tiles
	 */
	public SpriteSheetCounted(Image image,int tw,int th,int spacing) {
	 ...
	 */
	public SpriteSheetCounted(String ref,int tw,int th, int spacing) throws SlickException {
	 ...
	 * @throws SlickException Indicates a failure to load the image
	 */
	public SpriteSheetCounted(String ref,int tw,int th) throws SlickException {
		this(ref,tw,th,null);
	}
	 ...
	 * @throws SlickException Indicates a failure to load the image
	 */
	public SpriteSheetCounted(String ref,int tw,int th, Color col) throws SlickException {
	 ...
	 * @throws SlickException Indicates a failure to load the image
	 */
	public SpriteSheetCounted(String ref,int tw,int th, Color col, int spacing) throws SlickException {
	 ...
	 * @throws SlickException Indicates a failure to load the image
	 */
	public SpriteSheetCounted(String name, InputStream ref,int tw,int th) throws SlickException {
	...
	protected void initImpl() {
		...
		tilesAcross = ((getWidth()-(margin*2) - tw) / (tw + spacing)) + 1;
		tilesDown = ((getHeight()-(margin*2) - th) / (th + spacing)) + 1; 
	 ...
	 * 
	 * @param tile The position in tiles of the image to get
	 * @return The subimage at that location on the sheet
	 */
	public Image getSubImage(int tile) {
		init();
		
		if ((tile < 0) || (tile >= subImages.length)) {
			throw new RuntimeException("SubImage out of sheet bounds: "+tile);
		}
		
		return subImages[tile];
	}
	
	/**
	 * Get a sprite (new Image) at a particular cell on the sprite sheet
	 * 
	 ...
	 * @param tile The location of the cell to render
	 */
	public void renderInUse(int x,int y, int tile) {
		subImages[tile].drawEmbedded(x, y, tw, th);
	}
