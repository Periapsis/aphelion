#! /bin/sh
### BEGIN INIT INFO
# Provides:          aphelion server
# Required-Start:    $remote_fs $syslog
# Required-Stop:     $remote_fs $syslog
# Default-Start:     2 3 4 5
# Default-Stop:      0 1 6
# Short-Description: Aphelion server init script using jsvc
# Description:       Aphelion server init script using jsvc. Required packages: openjdk-7-jre-headless jsvc
### END INIT INFO
...
do_start()
{
	...
	start-stop-daemon --start --quiet --pidfile $PIDFILE --exec $DAEMON --chdir $APHELION_HOME --test > /dev/null \
		|| return 1
	start-stop-daemon --start --quiet --pidfile $PIDFILE --exec $DAEMON --chdir $APHELION_HOME -- \
		$DAEMON_ARGS \
		|| return 2
